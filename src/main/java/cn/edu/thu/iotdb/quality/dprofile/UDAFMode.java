...
public class UDAFMode implements UDTF {
  ...
  private void transformInt(Row row, PointCollector pc) {
    int v = row.getInt(0);
    intMap.addToValue(v, 1);
  }

  private void transformLong(Row row, PointCollector pc) {
    long v = row.getLong(0);
    longMap.addToValue(v, 1);
  }

  private void transformFloat(Row row, PointCollector pc) {
    float v = row.getFloat(0);
    floatMap.addToValue(v, 1);
  }

  private void transformDouble(Row row, PointCollector pc) {
    double v = row.getDouble(0);
    doubleMap.addToValue(v, 1);
  }

  private void transformBoolean(Row row, PointCollector pc) {
    boolean v = row.getBoolean(0);
    booleanCnt = v ? booleanCnt + 1 : booleanCnt - 1;
  }

  private void transformString(Row row, PointCollector pc) {
  ...
  private static class MaxSelector {
