...
public class MissDetector {
    ...
    private class MissingSubSeries implements Comparable<MissingSubSeries> {
        ...
        public MissingSubSeries(int start, int end) {
            this.start = start;
            this.end = end;
            this.slope = (getValue().get(end - 1) - getValue().get(start)) / (getTime().get(end - 1) - getTime().get(start));
        }

        @Override
        public String toString() {
            return String.format("Start: %d, End: %d, Length: %d, Slope: %.12f", start, end, end - start, slope);
        }
