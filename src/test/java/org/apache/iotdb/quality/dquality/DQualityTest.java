...
public class DQualityTest {
    ...
    public void test() throws IoTDBConnectionException, IOException, InterruptedException {
        final String[] udfList = {"UDTFCompleteness", "UDTFConsistency", "UDTFTimeliness", "UDTFValidity"};
        Gson gson = new Gson();
        Reader reader = Files.newBufferedReader(Paths.get("src/resources/DQualityQueries.json"));
        ...
        for (String functionName : udfList) {
            try { // register function
                ...
                session.executeNonQueryStatement(
                        "create function "
                                + functionName
                                + " as 'org.apache.iotdb.quality.dquality."
                                + functionName
                                + "'");
            } catch (Exception e) {
                try {
