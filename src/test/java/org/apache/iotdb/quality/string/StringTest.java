...
public class StringTest {
  ...
  public void test() throws IoTDBConnectionException, IOException, InterruptedException {
    final String[] udfList = {
      "UDTFRegexMatch", "UDTFRegexReplace", "UDTFRegexSplit", "UDTFStrReplace"
    };
    Gson gson = new Gson();
    Reader reader = Files.newBufferedReader(Paths.get("src/resources/StringQueries.json"));
    ...
    for (String functionName : udfList) {
      try { // register function
        ...
        session.executeNonQueryStatement(
            "create function "
                + functionName
                + " as 'org.apache.iotdb.quality.string."
                + functionName
                + "'");
      } catch (Exception e) {
        try {
