...
+ `rank`：所求分位数在所有数据中的排名百分比，取值范围为(0,1]。如当设为0.5时则计算中位数。
+ `error`(可选)：近似分位数的基于排名的误差百分比，取值范围为(0,1)。如`rank`=0.5且`error`=0.01，则计算出的分位数的真实排名百分比在0.49~0.51之间。当`error`=0时，计算结果为精确分位数。

**输出序列：** 输出单个序列，类型为DOUBLE，序列仅包含一个时间戳为0、值为近似分位数的数据点。
