...
本函数用于计算单列数值型数据的（精确/近似）绝对中位差，绝对中位差为所有数值与其中位数绝对偏移量的中位数。
...
+ `error`：近似绝对中位差的基于数值的预期误差上界，取值范围为[0,1)，默认值为0。如当`error`=0.01时，记精确绝对中位差为a，近似绝对中位差为b，不等式 $0.99a \le b \le 1.01a$ 成立。当`error`=0时，计算结果为精确绝对中位差。


**输出序列：** 输出单个序列，类型为DOUBLE，序列仅包含一个时间戳为0、值为近似绝对中位差的数据点。
...
计算近似绝对中位差时，可通过设置`error`来定义预期误差上界，若不设置`error`，则输出的结果误差上界无法保证。
