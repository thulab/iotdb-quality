...
+ `start`：表示所求数据范围的下限，默认值为-0x1.fffffffffffffP+1023。
+ `end`：表示所求数据范围的上限，默认值为0x1.fffffffffffffP+1023，start的值必须小于或等于end。
+ `count`: 表示直方图分桶的数量，默认值为10，其值必须为正整数

**输出序列：** 直方图分桶的值，其中第i个桶表示的数据范围为[ start+(i-1)*((end-start)/count), start+i*((end-start)/count) )
